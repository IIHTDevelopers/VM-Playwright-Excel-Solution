...
export default class ProcurementPage {
  ...
  async verifyRequestedDateColumnDateWithinRange(): Promise<boolean> {
    let isValid = true; // Initialize the validation flag

    try {
      // Define the Excel file path, sheet name, and column names for From and To Dates
      ...
      const sheetName = "DateRange"; // Adjust to the correct sheet name
      const columnNames = ["FromDate", "ToDate"]; // Columns with date range data

      // Read FromDate and ToDate from the Excel file using the readExcelData method
      const dateData = await CommonMethods.readExcelData(
        filePath,
        sheetName,
        columnNames
      );
      const firstRow = dateData[0]; // Assume data starts from the first row
      const fromDate = firstRow["FromDate"];
      const toDate = firstRow["ToDate"];

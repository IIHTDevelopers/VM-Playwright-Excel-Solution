...
export default class RadiologyPage {
  ...
  async performRadiologyRequestAndHandleAlert(): Promise<boolean> {
    let isAlertHandled = false;
    try {
      // Define the Excel file path, sheet name, and column name for From Date
      ...
      const columnNames = ["FromDate"]; // Column with From Date data

      // Read FromDate from the Excel file using the readExcelData method
